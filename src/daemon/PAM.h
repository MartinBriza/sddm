...
namespace SDDM {
    ...
    public:
         ...
         */
        PamService(const char *service, const QString &user, const QString &password, bool passwordless, QObject *parent = 0);
    ...
    private:
        ...
        /**
         * Internal state representation, making it possible to return to a state when it's needed
         */
        enum State {
            S_CLEAN = 0,
            S_STARTED,
        ...
        /**
         * \param state entry point
         * \return previous state
         */
        inline int statePrev(int state) {
            return (state > S_CLEAN) ? (state - 1) : (state);
        }

        /**
         * \return previous state to the current one
         */
        inline int statePrev() {
            return statePrev(m_state);
        }

        /**
         * \return next step to the current one
         */
        inline int stateNext() {
            return (m_state < S_AFTER_LAST) ? (m_state + 1) : (m_state);
        }

        /**
         * Cleans up the authenticator up until the given point
         *
         * Not a part of the PAM API, basically reverts the steps needed
         */
        bool cleanup(int point);

        int m_state { S_CLEAN }; ///< Current state storage
